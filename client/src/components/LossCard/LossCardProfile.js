    ...
    <div className={styles.cardContainer} onClick={handleCardClick} style={{ cursor: 'pointer' }}>
      <div className={styles.imageContainer}>
        ...
        {date && (
          <div className={styles.dateLabel}>
            <Image src={calendar} alt="Calendar" className={styles.calendarIcon} />
            {date}
          </div>
        )}
      </div>
      <div className={styles.contentContainer}>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div>
            <Button variant="link" style={{fontSize: 22}} onClick={onEdit} title="Редагувати">✏️</Button>
            <Button variant="link" style={{fontSize: 22}} onClick={onDelete} title="Видалити">🗑️</Button>
          </div>
        </div>
