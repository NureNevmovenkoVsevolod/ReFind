...
const buildQueryOptions = ({ type, category, limit, offset }) => ({
    ...
    include: [
        ...
        { model: Category, attributes: ["categorie_id", "categorie_name"], ...(category && { where: { categorie_name: category } }) },
    ],
...
class AdvertisementController extends IAdvertisementController {
    constructor() {
        ...
        this.getAdvertisementsForModeration = this.getAdvertisementsForModeration.bind(this);
        this.moderateAdvertisement = this.moderateAdvertisement.bind(this);
    }

    async createAdvertisement(req, res) {
        try {
            const { title, description, categorie_id, location_description, location_coordinates, reward, type, phone, email, incident_date } = req.body;
            const advertisement = await Advertisement.create({
                title, description, categorie_id, user_id: req.user.id, location_description,
                location_coordinates: JSON.stringify(location_coordinates), reward: reward || 0, type, status: "active", phone, email, mod_check: false, incident_date,
            });
            if (req.files?.length) {
                await Promise.all(req.files.map(async (file) => {
                    try {
                        const result = await new Promise((resolve, reject) => {
                            const stream = cloudinary.uploader.upload_stream({ folder: "advertisements" }, (error, result) => (error ? reject(error) : resolve(result)));
                            stream.end(file.buffer);
                        });
                        await Image.create({ advertisement_id: advertisement.advertisement_id, image_url: result.secure_url });
                    } catch (error) { console.error("Error creating image record:", error); }
                }));
            }
            const completeAd = await Advertisement.findOne({ where: { advertisement_id: advertisement.advertisement_id }, include: [{ model: Image, attributes: ["image_url"] }] });
    ...
    async getAllAds(req, res) {
        try {
            ...
            const whereClause = { status: "active", mod_check: true };
            if (category) whereClause.categorie_id = category;
    ...
    async getAdsByType(type, req, res) {
        try {
            ...
            const offset = (page - 1) * limit;
            const category = req.query.category;
            const { count, rows } = await Advertisement.findAndCountAll(buildQueryOptions({ type, category, limit, offset }));
            res.json({
                items: rows.map(mapAdWithCategory),
                total: count,
                currentPage: page,
    ...
    async getAdvertisementById(req, res) {
        try {
            ...
            res.json({ ...ad.toJSON(), categorie_id: ad.categorie_id, categorie_name: ad.Category?.categorie_name, location_coordinates: JSON.parse(ad.location_coordinates) });
        } catch (error) {
    ...
    async updateAdvertisement(req, res) {
        try {
            const ad = await Advertisement.findOne({ where: { advertisement_id: req.params.id, user_id: req.user.id } });
            if (!ad) return res.status(404).json({ message: "Advertisement not found or unauthorized" });
            if (ad.mod_check === false) return res.status(403).json({ message: "Оголошення знаходиться на модерації і не може бути відредаговане." });
            const { title, description, categorie_id, location_description, location_coordinates, reward, phone, email } = req.body;
            await ad.update({ title, description, categorie_id, location_description, location_coordinates: JSON.stringify(location_coordinates), reward: reward || 0, phone, email, mod_check: false });
            let newImageUrls = [];
            if (req.body.existingImages) newImageUrls = Array.isArray(req.body.existingImages) ? req.body.existingImages : [req.body.existingImages];
            if (req.files?.length) {
            ...
            if (newImageUrls.length > 0) {
                ...
                await Image.bulkCreate(newImageUrls.map((url) => ({ advertisement_id: ad.advertisement_id, image_url: url })));
            }
            ...
            res.json({ ...updatedAd.toJSON(), categorie_id: updatedAd.categorie_id, categorie_name: updatedAd.Category?.categorie_name, location_coordinates: JSON.parse(updatedAd.location_coordinates) });
        } catch (error) {
    ...
    async deleteAdvertisement(req, res) {
        try {
            const ad = await Advertisement.findOne({ where: { advertisement_id: req.params.id, user_id: req.user.id }, include: [{ model: Image, attributes: ["image_url"] }] });
            if (!ad) return res.status(404).json({ message: "Advertisement not found or unauthorized" });
    ...
    async addImagesToAdvertisement(req, res) {
        try {
            ...
            const ad = await Advertisement.findOne({ where: { advertisement_id, user_id: req.user.id } });
            if (!ad) return res.status(404).json({ message: "Advertisement not found or unauthorized" });
            if (req.files?.length) {
                await Promise.all(req.files.map(async (file) => {
                    const result = await new Promise((resolve, reject) => {
                    ...
                    await Image.create({ advertisement_id: ad.advertisement_id, image_url: result.secure_url });
                }));
                const updatedAd = await Advertisement.findOne({ where: { advertisement_id }, include: [{ model: Image, attributes: ["image_url"] }] });
    ...
    async getAdvertisementsForModeration(req, res) {
        try {
            ...
            res.status(500).json({ message: "Failed to fetch advertisements for moderation", error: error.message });
        }
    }
    ...
    async moderateAdvertisement(req, res) {
        try {
            ...
            const advertisement = await Advertisement.findOne({ where: { advertisement_id: id } });
            if (!advertisement) return res.status(404).json({ message: "Оголошення не знайдено" });
            if (approved) {
